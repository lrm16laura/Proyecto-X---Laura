...
def procesar_logica_estable(df_dem, df_mat, df_cli, df_cap, ajustes_semanales):
    ...
    for _, fila in df_g.iterrows():
        ...
        for lote in lotes:
            ...
            # si el centro tiene capacidad cero base -> no avanzar días
            if base_cap_por_centro.get(str(pref),0)==0:
...
# --- TAB 2 ---
with tab2:
    if None in [df_cap,df_mat,df_cli,df_dem]:
        ...
        st.subheader("⚙️ Configuración de empates")
        ...
        if st.button("🚀 Ejecutar cálculo de propuesta",use_container_width=True):
            ...
            # Mostrar tabla SIN "Sin capacidad"
